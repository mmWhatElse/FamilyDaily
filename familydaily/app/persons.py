"""Familienmitglieder — persons API incl. HA calendar mapping."""
...
async def create_person(data: PersonIn):
    ...
    try:
        cur = await db.execute(
            "INSERT INTO person (name, color, emoji, calendar_entity_id) VALUES (?, ?, ?, ?)",
...
async def patch_person(person_id: int, data: PersonPatch):
    ...
    try:
        ...
        for field in ("name", "color", "emoji", "calendar_entity_id"):
